...
(function (window, undefined) {
    ...
    function previewImage(box, task, callback) {
        ...
        if (file) {
            ...
            readAsURL(file, function (src) {
                if (src) box.innerHTML = '<img src="' + src + '" /><button class="removeBtn" onclick="javascript:removeImg(this)">x</button>';
            ...
            if (src) {
                box.innerHTML = '<img src="' + src + '" /><button class="removeBtn" onclick="javascript:removeImg(this)">x</button>';
    ...
    Uploader.extend({
        ...
        over: function (task) {
            ...
            console.log($('.u-item').length);
            if($('.u-item').length >= 3){$('.x-button').css('display','none')}
            else{$('.x-button').css('display','inline-block')}
        }
    });
